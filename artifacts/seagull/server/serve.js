/**
 * Standalone production server for Expo static builds + DELIVERY TRACKING
 */
...
let driverLocation = { lat: 30.0444, lng: 31.2357 };
  ...
  try {
    const response = await fetch(
      "https://api.openrouteservice.org/v2/directions/driving-car",
      {
        ...
        headers: {
          Authorization:
            "eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6ImE2MTRlMjAyMDdkYjQzZjE4NTRjYWI4N2VhYmVmZjM2IiwiaCI6Im11cm11cjY0In0",
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          coordinates: [
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === "/driver-location" && req.method === "GET") {
    res.writeHead(200, { "content-type": "application/json" });
    return res.end(JSON.stringify(driverLocation));
  }
  ...
  if (pathname === "/update-location" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        driverLocation = JSON.parse(body);
      } catch {}
